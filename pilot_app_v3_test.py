...
GROUPS = {
    "group_A": {
        "task2": ["OCD", "MDD", "None_T2", "GAD"],  # 질환 종류 맞추기용 4개
        "task1": ["Moderate", "Mild", "Severe", "None_T1"] # 우울증 심각도 평가용 4개
    }
}
...
def participant_view():
    ...
    # ---------------------------------------------------------
        ...
        with st.form("demography"):
            ...
            st.session_state.data['gender'] = st.radio("**2. 성별**", options=["남", "여"], index=None, horizontal=True)
            ...
            st.session_state.data['consulted_disorders'] = st.text_input("**6.1 실제 면담 경험이 있다면, 어떤 질환의 환자를 면담해 보셨나요?**")
            st.session_state.data['clinical_years'] = st.number_input("**6.2 임상 경력 년차 (※ 6. 상담 경험 '예' 응답자만 기재)**", min_value=0, max_value=50, value=0, step=1)
            ...
            if st.form_submit_button("실험 시작하기"):
                ...
                st.session_state.data['training_experiences'] = selected_experiences
                # 검증 완료 후 이동
    ...
    # ---------------------------------------------------------
        ...
        # [수정됨] 논리적 오류 교정: Task 2은 task1_videos 목록 사용
...
def render_system_evaluation_form(video_id, task_num, v_idx):
    with st.form(f"survey_part2_{video_id}_{v_idx}"):
        ...
        st.session_state.data[f"{video_id}_q23_reasoning1"] = st.radio("**8. 나는 이 상담 영상을 시청하는 동안, 환자의 상태를 파악하기 위해 필요한 정보를 주의 깊게 확인하였다.**", likert_scales, index=None, horizontal=True)
        st.session_state.data[f"{video_id}_q24_reasoning2"] = st.radio("**9. 나는 이 상담 영상을 시청하는 동안, 새로운 정보를 확인할 때마다 환자의 상태에 대한 나의 판단을 수정하였다.**", likert_scales, index=None, horizontal=True)
